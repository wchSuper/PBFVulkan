...
layout(binding=1) uniform sampler2D TexSide;
layout(binding=2) uniform sampler2D TexTop;
...
void main() {
	...
	vec3 blending = pow(abs(worldNormal), vec3(4.0)); // The power sharpens the blend
	blending /= dot(blending, vec3(1.0));

	// 2. Texture coordinates from three planes (XY, YZ, XZ) based on world position
	// You might need to scale these coordinates by dividing by a 'textureScale' uniform or constant
	vec2 texCoordX = FragPos.yz;
	vec2 texCoordY = FragPos.xz;
	vec2 texCoordZ = FragPos.xy;

	// 3. Sample the texture from three directions
	vec4 colorX = texture(TexSide, texCoordX);
	vec4 colorY = texture(TexTop, texCoordY);
	vec4 colorZ = texture(TexSide, texCoordZ);
